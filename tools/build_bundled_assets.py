...
def animation_from_atlas(atlas_path, destination):
  image = Image.open(atlas_path).convert("RGBA")
  frames = [image.crop((column * CELL_W, 0, (column + 1) * CELL_W, CELL_H)) for column in range(6)]
  frames[0].save(
    destination,
    "GIF",
    save_all=True,
...
def main():
  ...
  for theme in THEMES:
    ...
    animation = pet_dir / "pet.gif"
    atlas(theme, sprite)
    animation_from_atlas(sprite, animation)
    write_json(pet_dir / "pet.json", {"id": theme["pet"], "displayName": theme["name"] + "搭档", "spritesheetPath": "spritesheet.png", "animationPath": "pet.gif"})
    ...
    write_json(pack / "rights.json", {"schemaVersion": 1, "status": "approved", "basis": "procedurally-generated-original", "license": "CC0-1.0", "source": "Generated by tools/build_bundled_assets.py from basic geometric primitives.", "assets": [{"path": "pet/spritesheet.png", "sha256": sha(sprite)}, {"path": "pet/pet.gif", "sha256": sha(animation)}]})
